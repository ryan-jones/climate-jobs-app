...
def create_job_sectors(cursor, job_id, sector):
    ...
    if result is None:
        current_app.logger.warning(f'None value for {formatted_sector}')
        return (job_id, None)
    sector_id = result[0]
...
def get_jobs(cursor, filters):
    try:
        ...
        current_app.logger.error(f'failed to retrieve jobs records => {error}')


@init_db
def update_jobs_list(cursor, app):
    try:
        with app.app_context():
            app.logger.info('Retrieving latest climate jobs')
            data = get_climate_jobs()
            jobs = data['jobs']
            app.logger.info('Deleting old climate job records')
            cursor.execute(DELETE_JOB_SECTORS)
            cursor.execute(DELETE_JOBS)  # removes old job data
            app.logger.info('Inserting new climate job records')
            ...
            app.logger.info('Insertion of new job records complete')
            return {"message": f"{data['count']} jobs added"}, 201
    except (psycopg2.Error) as error:
        app.logger.error(f'failed to insert jobs records => {error}')
...
def get_job_total_count(cursor):
    try:
        ...
        current_app.logger.error(f'failed to retrieve jobs count => {error}')
